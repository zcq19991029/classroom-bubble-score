...
- 单击学生气泡：增加 1 次课堂参与。
- 双击学生气泡：撤销 1 次。
- 开始上课前填写“本节任务数”；每完成一个任务记 1 分，该节课的有效课堂分不会超过任务数，避免误点造成排名失真。
- 如果误点或需要课后补分，在“成绩后台 → 加减分”使用左侧 `−1` 或右侧 `＋1`；系统会按日志重新求和，不再依赖可能失真的缓存总分。
- “操作日志”只负责审计，逐条显示北京时间、课次、学生、加减分及来源（课堂气泡/后台补录），方便判断是课内操作还是课后修正。
- 并列名次采用标准竞赛排名：16 人并列第 1 时，下一人是第 17；全班尚无课堂参与时学习表现为 0，不会默认获得 80 分。
- 平板、希沃和窄屏下成绩后台自动铺满可用视口，栏目和表格在各自区域滚动，无需调整浏览器缩放。
...
```text
考勤分 = 20 × max(0, 本学期计划课次 - 扣减单位) ÷ 本学期计划课次
```

当前课表每个班共 24 次课（1–2 或 1–4 连堂均算一次课），系统默认填入 24。迟到 1 次扣 1 单位，旷课 1 次扣 3 单位；计划课次仍可在综合考核后台修改。
...
- 分课次记录：加减分日志按“某节课＋某学生”汇总，考勤按日期、课次分组。
- 记录纠错：可撤销某生本节最近 1 分、清空某生本节或整节课记录；每次都按剩余日志重算总分。
- 平板重力感应：点击“重力感应”并授权后，气泡会随设备倾斜方向漂移；不支持传感器的电脑自动保留原物理动画。
...
6. 点击“成绩后台”查看综合考核、课堂排名、加减分、操作日志和考勤记录。
